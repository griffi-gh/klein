...
namespace klein::camera {
    void Camera2d::update_view() {
        ...
        switch (config.size_option) {
            case SizeOption::None:
                vp_size = sf::Vector2f(viewport.getSize()) * config.scale;
                break;
            case SizeOption::FitInside:
                if (aspect > 1.) {
                    vp_size = sf::Vector2f(config.base_size.x, config.base_size.x / aspect) * config.scale;
                } else {
                    vp_size = sf::Vector2f(config.base_size.y * aspect, config.base_size.y) * config.scale;
                }
                break;
    ...
    void Camera2d::update(const entt::registry& registry, sf::Time dt) {
        ...
        if (config.smooth) {
            center_pos += dt.asSeconds() * config.smooth_fac * (subject_pos - center_pos);
        } else {
            center_pos = subject_pos;
        }
        update_view();
    }
